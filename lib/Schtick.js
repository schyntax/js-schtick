...
function Schtick (format, options, task)
{
	...
	/** @member {{window: number, ideal: boolean}} */
	this.options = { window: 0, ideal: false, autoStart: false };
...
Schtick.prototype.start = function (lastKnownEvent)
{
	...
	if (lastKnownEvent instanceof Date)
		this.lastEvent = lastKnownEvent;
	
	var now = new Date();
	var event = this.schedule.next(now);
	
	if (this.lastEvent && this.options.window > 0)
	{
		var prev = this.schedule.previous(now);
		if (prev > (now - this.options.window) && prev > this.lastEvent)
		{
			event = prev;
		}
	}
...
{
	this.lastEvent = this.options.ideal ? intendedTime : new Date();
	this.__timeout = null;
	this.start();
